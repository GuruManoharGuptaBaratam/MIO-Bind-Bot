...
|---|---|---|---|
| **kansei** | 2-click | Captures the surroundings and returns a spoken scene description via a cloud VLM pipeline | ✅ Implemented |
...
The `kansei` command's scene description is generated through a cloud pipeline:

1. The ESP32-CAM captures an image and uploads it over Wi-Fi to a serverless endpoint.
2. The backend runs a vision-language model to generate a scene description.
3. The description is converted to speech and streamed back to the device.

**VLM:** [`HuggingFaceTB/SmolVLM-500M-Instruct`](https://huggingface.co/HuggingFaceTB/SmolVLM-500M-Instruct) — model integration files to be added.

The serverless backend is deployed on [Lightning.ai](https://lightning.ai/), chosen for straightforward serverless hosting suited to intermittent, on-demand usage.
...
├── cloud/                    # Serverless backend (kansei VLM pipeline)
├── docs/
│   ├── assets/
...
> - A Lightning (deployment) account for the `kansei` VLM backend
