...
esp_err_t sd_storage_init(void)
{
    ...
    // Explicitly define pins to prevent SD driver from probing GPIO 12 & 13 (Servos)
                                 ...
                                 char *out_path, size_t out_path_len)
{
    ...
    static int s_seq = 0;
    char path[64];
    snprintf(path, sizeof(path), MOUNT_POINT "/kiroku_%04d.avi", s_seq++);
    ...
    ESP_LOGI(TAG, "opened %s (%lux%lu @%dfps)", path,
